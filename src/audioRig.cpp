...
std::uint64_t AudioRig::addLowPass(Vector2 position)
{
	LowPass component;
	component.position = position;
	components[idCounter] = std::make_unique<LowPass>(component);
	return idCounter++;
}

std::uint64_t AudioRig::addReverb(Vector2 position)
{
	Reverb component;
	component.position = position;
